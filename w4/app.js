...
class MovieLensApp {
    ...
    async loadData() {
        ...
        try {
            ...
            itemsLines.forEach(line => {
                ...
                itemGenres.set(itemId, movieGenres);
            });
            ...
            this.updateStatus(`Loaded ${this.interactions.length} interactions, ${this.items.size} movies, and ${this.genres.size} genres. ${this.userTopRated.size} users have 20+ ratings.`);
    ...
    async train() {
        ...
        this.updateStatus('Initializing Deep Learning Two-Tower model with MLP...');
        
        // Initialize model with MLP architecture
        ...
        const genreIndices = this.interactions.map(i => i.genreIds.length > 0 ? i.genreIds[0] : 0);
    ...
    updateLossChart() {
        ...
        ctx.fillText(`Current: ${this.lossHistory[this.lossHistory.length - 1].toFixed(4)}`, 10, 40);
    }
    ...
    async visualizeEmbeddings() {
        ...
        try {
            ...
            const embeddings = embeddingsTensor;
            ...
            // Draw points with colors based on genres
            sampleIndices.forEach((itemIdx, i) => {
                const itemId = this.reverseItemMap.get(itemIdx);
                const item = this.items.get(itemId);
                const primaryGenre = item.genreIds.length > 0 ? item.genreIds[0] : 0;
                ...
                const hue = (primaryGenre / this.genres.size) * 360;
            ...
            ctx.fillStyle = '#2c3e50';
            ctx.font = '14px Arial';
            ctx.fillText('Deep Learning Item Embeddings (PCA)', 10, 20);
            ctx.font = '12px Arial';
            ctx.fillText(`Showing ${sampleSize} items colored by genre`, 10, 40);
    ...
    async test() {
        ...
        try {
            ...
            // Get user embedding through MLP tower
    ...
    displayResults(userId, userInteractions, recommendations) {
        ...
        let html = `
            <h2>Deep Learning Recommendations for User ${userId}</h2>
